...
开发机上：
  ...
  3) 构建并导出变更镜像（文件名带版本号，放入 images/）：
     docker build -t gongwen-web:<新版本> -f gongwen-docker/Dockerfile .
  ...
  ⚠️ nginx conf 与 runtime-config 生成脚本均烘焙在前端镜像内
     （gongwen-docker/default-prod.conf、gongwen-docker/docker-entrypoint.d/）：
     改这两者必须重打前端镜像，只改 compose 或只 reload 不会生效。

----------------------------------------
从旧版（gongwen-docker/ 挂载式 compose）升级
----------------------------------------
