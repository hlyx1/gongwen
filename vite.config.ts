...
// 函数式配置：单文件模式经 mode 判定（task-0003 待办-0020，裁定1 改道）
// 双入口保持等价：npm run build:single（--mode single）或 SINGLE_FILE=1 npx vite build
export default defineConfig(({ mode }) => {
  const isSingleFile = mode === 'single' || !!process.env.SINGLE_FILE
