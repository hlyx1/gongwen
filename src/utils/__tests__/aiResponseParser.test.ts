...
describe('parseTableRow', () => {
  it('解析有效的表格行', () => {
    const result = parseTableRow('| 1 | 原句内容 | 修改建议 |')
...
describe('思考内容清除', () => {
  describe('流式解析 - 无思考标签', () => {
    it('正常解析没有思考标签的内容', () => {
      ...
      const sentenceMap = createTestSentenceMap()
      ...
      for (let i = 0; i < lines.length; i++) {
        const parsed = parseStreamingLine(state, lines[i], sentenceMap)
      ...
      const flushResult = flushParser(state, sentenceMap)
      for (let j = 0; j < flushResult.results.length; j++) {
  ...
  describe('flushParser - 缓冲区中有思考标签', () => {
    it('刷新时正确处理缓冲区中的思考内容', () => {
      ...
      const result = flushParser(state, sentenceMap)
...
describe('边界情况', () => {
  it('空输入返回 null', () => {
    ...
    const parsed = parseStreamingLine(state, '', sentenceMap)
