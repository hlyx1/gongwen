 ...
 * 渲染单个章节内容
 */
function SectionContent({ section, level }: { section: Section; level: number }) {
  ...
  return (
    <div id={`section-${section.id}`} className="standard-section">
      ...
      {section.children && section.children.length > 0 && (
        <div className="standard-section-children">
          {section.children.map((child) => (
            <SectionContent key={child.id} section={child} level={level + 1} />
          ))}
        </div>
      )}
    </div>
...
export function StandardModal({ onClose }: StandardModalProps) {
  ...
  useEffect(() => {
    ...
    const flatSections = flattenSections(gb9704Data)
    const sections = flatSections.map((s) => document.getElementById(`section-${s.id}`)).filter(Boolean)
    ...
    <div className="standard-overlay" onClick={onClose}>
      <div className="standard-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="standard-header">
          <h2 className="standard-title">党政机关公文格式</h2>
          <span className="standard-subtitle">GB/T 9704-2012</span>
        ...
        <div className="standard-body">
          ...
          <div className="standard-content" ref={contentRef}>
            {gb9704Data.map((section) => (
              <SectionContent key={section.id} section={section} level={0} />
            ))}
          </div>
        </div>
        ...
        <div className="standard-footer">
          <span className="standard-footer-note">本标准由 中国国家标准化管理委员会发布</span>
