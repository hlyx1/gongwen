...
/** 日期警告阈值（天数） */
const DATE_WARNING_THRESHOLD = 7
...
function chineseToNumber(str: string): number | null {
  ...
  // 单个数字
  if (str.length === 1) {
    const idx = CHINESE_DIGITS.indexOf(str)
  ...
  if (str.charAt(0) === '十') {
    ...
    const rest = chineseToNumber(str.slice(1))
    return rest ? 10 + rest : null
  }
  
  // 处理 "二十"~"九十九"
  if (str.length === 2 && str.charAt(1) === '十') {
    const tens = chineseToNumber(str.charAt(0))
  ...
  if (str.length >= 2 && str.indexOf('十') > 0) {
    ...
    if (parts.length === 2) {
      const tensPart = chineseToNumber(parts[0])
      const onesPart = parts[1] ? chineseToNumber(parts[1]) : 0
...
function parseHeadingNumber(content: string, level: number): number | null {
  ...
  if (level === 1) {
    // 一级标题：一、二、三...
    const match1 = content.match(/^([一二三四五六七八九十]+)、/)
    if (match1) return chineseToNumber(match1[1])
  } else if (level === 2) {
    // 二级标题：（一）、（二）...
    const match2 = content.match(/^（([一二三四五六七八九十]+)）/)
    if (match2) return chineseToNumber(match2[1])
  } else if (level === 3) {
    // 三级标题：1.、2.、3...
    const match3 = content.match(/^(\d+)\./)
    if (match3) return parseInt(match3[1], 10)
  } else if (level === 4) {
    // 四级标题：（1）、（2）...
    const match4 = content.match(/^（(\d+)）/)
...
function checkDateWarning(dateContent: string): DateWarning | null {
  const date = parseChineseDate(dateContent)
  if (!date) return null
...
function checkHeadingLevel(body: DocumentNode[]): HeadingLevelWarning | null {
  const issues: Array<{ content: string; level: number; missingParentLevel: number }> = []
...
function checkHeadingNumber(body: DocumentNode[]): HeadingNumberWarning | null {
  const issues: HeadingNumberIssue[] = []
  
  let h1Expected = 1  // 一级标题期望序号
  let h2Expected = 1  // 当前一级标题下的二级标题期望序号
  let h3Expected = 1  // 当前二级标题下的三级标题期望序号
  let h4Expected = 1  // 当前三级标题下的四级标题期望序号
  
  for (let i = 0; i < body.length; i++) {
    const node = body[i]
    
    if (node.type === NodeType.HEADING_1) {
      const actualNum = parseHeadingNumber(node.content, 1)
      ...
      const actualNum2 = parseHeadingNumber(node.content, 2)
      ...
      const actualNum3 = parseHeadingNumber(node.content, 3)
      ...
      const actualNum4 = parseHeadingNumber(node.content, 4)
...
export function useDetectionData(ast: GongwenAST): DetectionResult {
  return useMemo(function () {
    ...
    const bodyStats = calculateBodyStats(ast.body)
    const headingLevelWarning = checkHeadingLevel(ast.body)
    const headingNumberWarning = checkHeadingNumber(ast.body)

    // 1. 公文标题（支持多段标题）
    if (ast.title.length > 0) {
      // 多段标题用换行符连接显示
      const titleContent = ast.title.map(function(node) { return node.content }).join('\n')
      points.push(
    ...
    // 2. 主送机关
    const addresseeNode = findNodeByType(ast.body, NodeType.ADDRESSEE)
    if (addresseeNode) {
      points.push(
        createDetectionPoint(
    ...
    // 3. 正文内容
    let bodyStatus = bodyStats.charCount > 0 ? DetectionStatus.DETECTED : DetectionStatus.MISSING
    ...
    // 4. 发文机关署名
    const signatureNode = findNodeByType(ast.body, NodeType.SIGNATURE)
    if (signatureNode) {
      points.push(
        createDetectionPoint(
    ...
    // 5. 成文日期
    const dateNode = findNodeByType(ast.body, NodeType.DATE)
    if (dateNode) {
      const dateWarning = checkDateWarning(dateNode.content)
      if (dateWarning) {
        points.push(
          createDetectionPoint(
