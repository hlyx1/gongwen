...
var RUNTIME_CONFIG_GLOBAL = '__GONGWEN_RUNTIME_CONFIG__';

/** DEV 编译默认组（npm run dev / vitest）：外网 DeepSeek，密钥由 vite 代理注入故前端留空 */
var DEFAULTS_DEV: AIServiceConfig = {
...
var DEFAULTS_PROD: AIServiceConfig = {
  baseUrl: '/llm/v1/chat/completions',
...
function applyRuntimeOverride(selected: AIServiceConfig): AIServiceConfig | null {
  var holder = globalThis as unknown as Record<string, unknown>;
  var raw = holder[RUNTIME_CONFIG_GLOBAL];
  if (raw === null || typeof raw !== 'object') {
    return selected;
  }
  var cfg = raw as Record<string, unknown>;
...
export function getAIServiceConfig(): AIServiceConfig | null {
  ...
  var selected = import.meta.env.DEV ? DEFAULTS_DEV : DEFAULTS_PROD;

  var merged = applyRuntimeOverride(selected);
  if (merged === null) {
    return null;
  }
