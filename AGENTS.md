...
│   └── aiProofreadService.ts  # 提示词构建、流式请求发送
├── types/               # TypeScript 类型定义
│   ├── ast.ts           # AST 节点类型
│   ├── detection.ts     # 检测点相关类型
│   ├── aiProofread.ts   # AI 审核类型（含 AIServiceConfig）
...
└── public/              # 静态资源
    ├── runtime-config.js      # 运行时部署配置默认 no-op（生产容器启动时按 env 重新生成）
    ├── 正确.svg         # 检测点正确状态图标
    ├── 暂无.svg         # 检测点缺失状态图标
    └── 警告.svg         # 检测点警告状态图标

backend/                 # 统计后端（FastAPI，生产镜像 gongwen-stats）
